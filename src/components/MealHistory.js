      ...
      <div className='px-4 pb-4'>
        <div className='flex gap-3'>
          ...
          <div className='flex-1 min-w-0'>
            ...
            <div className='flex flex-wrap gap-1.5 mt-3'>
              {/* Consolidated Period Badge */}
              <div className='inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-[#F2FAF3] text-[#065c2d]'>
                <span className='text-[10px]'>{period.icon}</span>
                <span className='text-[9px] font-bold'>
                  {period.label} {period.time ? `• ${period.time}` : ''}
                </span>
              </div>
...
export default function MealHistory () {
  ...
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const [filterDate, setFilterDate] = useState('') // New state for Date Picker
  ...
  const loadHistory = async () => {
    try {
      ...
      if (response?.success) {
        // Automatically remove duplicate history records
        const apiHistory = Array.isArray(response.data) ? response.data : []
        const uniqueHistory = Array.from(
          new Map(
            apiHistory.map(meal => [
              meal.mapping_id || `${meal.product_id}-${meal.delivery_date}`, // Fallback unique key
              meal
            ])
          ).values()
        )
  ...
  const displayHistory = useMemo(() => {
    ...
    return history.filter(meal => {
      try {
        const mealDate = new Date(meal.delivery_date)
  ...
  // Update stats based on the FILTERED displayHistory instead of all history
  const consumedCount = displayHistory.filter(
    meal => String(meal.status).toUpperCase() === 'CONSUMED'
  ).length

  const skippedCount = displayHistory.filter(
    meal => String(meal.status).toUpperCase() === 'SKIPPED'
  ).length
  ...
  return (
    <div className='min-h-screen bg-[#F7F8F7] pb-24 font-sans'>
      ...
      <div className='max-w-md mx-auto px-5 pt-5'>
          ...
          <>
            <div className='flex items-start justify-between mb-4'>
              ...
              <div className='px-3 py-1.5 rounded-full bg-[#EEF8F1] text-[#065c2d] text-[10px] font-bold'>
                ...
                {displayHistory.length === 1 ? 'Meal' : 'Meals'}
              </div>
            </div>
            ...
            <div className='mb-5 flex items-center gap-2'>
              <div className='relative flex-1'>
                <input
                  ...
                  className='w-full h-11 bg-white border border-gray-200 rounded-xl px-4 text-[13px] text-gray-700 outline-none focus:border-[#065c2d] focus:ring-1 focus:ring-[#065c2d] transition-all'
                />
              </div>
              {filterDate && (
                <button
                  ...
                  className='h-11 px-4 rounded-xl bg-gray-100 text-gray-600 text-[12px] font-bold hover:bg-gray-200 transition-colors'
                >
                  Clear
                </button>
              )}
        ...
        {/* Empty State (Total History Empty) */}
        {!loading && !error && history.length === 0 && (
          <div className='bg-white rounded-[22px] border border-gray-100 p-8 text-center shadow-sm'>
            <div className='w-20 h-20 rounded-full bg-[#EEF8F1] flex items-center justify-center mx-auto text-3xl'>
              🍽️
        ...
        {!loading &&
          ...
          displayHistory.length === 0 && (
            <div className='bg-white rounded-[22px] border border-gray-100 p-8 text-center shadow-sm my-4'>
              <div className='text-3xl mb-3'>📅</div>
              <h2 className='text-[16px] font-extrabold text-gray-900'>
                No meals on this date
              </h2>
        ...
        {!loading && !error && displayHistory.length > 0 && (
          <div className='space-y-4'>
            {displayHistory.map(meal => (
              <HistoryCard
                key={
                  meal.mapping_id || `${meal.product_id}-${meal.delivery_date}`
                }
                meal={meal}
              />
            ))}

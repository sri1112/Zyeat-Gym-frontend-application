...
export default function PaymentsModal({
  ...
  // Refined status badges for a cleaner premium look
  const getStatusStyle = (status) => {
    const base = 'px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider border'
    switch (status) {
      case 'SUCCESS': return `${base} bg-green-50/50 text-green-700 border-green-100`
      case 'PENDING': return `${base} bg-amber-50/50 text-amber-700 border-amber-100`
      case 'FAILED': return `${base} bg-red-50/50 text-red-700 border-red-100`
  ...
  return (
    <div className='fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4'>
      ...
      <button
        type='button'
        onClick={onClose}
        className='absolute inset-0 bg-black/30 backdrop-blur-[4px] transition-all'
      />

      {/* Modal Container */}
      <div className='relative bg-white w-full sm:max-w-md h-[85vh] sm:h-[650px] rounded-t-[32px] sm:rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden animate-slide-up'>
        
        {/* Header */}
        <div className='flex items-center justify-between px-6 py-6'>
          <div>
            <h2 className='text-[22px] font-black text-gray-900 tracking-tight'>Payment History</h2>
            <p className='text-[12px] text-gray-400 font-medium'>Transactions & Details</p>
          </div>
          <button
            type='button'
            onClick={onClose}
          ...
          {loading ? (
            ...
            payments.map((payment) => (
              <button
                ...
                type='button'
                onClick={() => onSelectPayment(payment.payment_id)}
                className='w-full text-left bg-white border border-gray-100 rounded-2xl p-5 shadow-[0_4px_12px_rgba(0,0,0,0.03)] hover:border-[#065c2d]/20 hover:shadow-[0_8px_20px_rgba(6,92,45,0.05)] transition-all duration-300 group'
              >
                <div className='flex justify-between items-start mb-0.5'>
                  <div className='flex items-center gap-3'>
                    <div className='w-12 h-12 rounded-2xl bg-[#F4F9F5] text-[#065c2d] flex items-center justify-center text-xl'>
                       {payment.payment_method === 'COD' ? '📦' : '💳'}
                    </div>
                    <div>
                      <p className='text-[16px] font-extrabold text-gray-900'>{formatAmount(payment.amount)}</p>
                      <p className='text-[11px] font-medium text-gray-400 uppercase tracking-wide'>{payment.payment_method || 'Online'}</p>
                    </div>
                  </div>
                  <span className={getStatusStyle(payment.payment_status)}>
                ...
                <div className='flex items-center justify-between pt-4 border-t border-gray-50'>
                  <div>
                    ...
                    <p className='text-[12px] font-bold text-gray-700'>{payment.order_number || '---'}</p>
                  </div>
                  <div>
                    <p className='text-[9px] font-bold text-gray-300 uppercase tracking-widest text-right'>Date</p>
                    <p className='text-[12px] font-bold text-gray-700'>{formatDate(payment.paid_at || payment.created_at)}</p>
                  </div>
                </div>
              </button>
            ))
...
const EmptyState = () => (
  <div className='h-full flex flex-col items-center justify-center pt-20'>
    <div className='w-20 h-20 rounded-3xl bg-gray-50 flex items-center justify-center text-4xl mb-6'>
      💸
    </div>
    <h3 className='text-[16px] font-bold text-gray-900'>No payments yet</h3>
    <p className='text-[12px] text-gray-400 mt-1'>Transactions will appear here</p>
  </div>
)